...
def test_sample_base64_iterator(df_path: Path, root: Path, distribution: str, seed: int) -> Generator[
...
def evaluate_model(root_face_folder: Path, df_faces_path: Path, arch: str, input_shape: Tuple[int, int, int],
                   output_folder: Path, distribution: str, seed: int, yunet_model_path: Path,
                   model_name: Optional[str]) -> Tuple[list, list, float]:
    ...
    print("Evaluating...")
    for frame, label in test_sample_base64_iterator(df_faces_path, root_face_folder, distribution, seed):
        start = time.time()
    ...
    return y_true, y_pred, sum_time / count
...
def launch_evaluation(
    ...
    y_true, y_pred, avg_prediction_time = evaluate_model(root_face_folder, df_faces_path, arch, input_shape,
                                                         output_folder, distribution, seed, yunet_model_path,
                                                         model_name)
